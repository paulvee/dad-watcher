...
VERSION = "2.2.2"  # added sending an email next to an SMS, as backup
#VERSION = "2.2.1" # added a little delay after movement detection to avoid jitters
...
fromaddr = conf["fromaddr"]    # your gmail account
toaddr = conf["toaddr"]        # your gmail address
gmail_password = conf["gmail_password"]  # your gmail password
...
if DEBUG :
    ...
    print("fromaddr is {}".format(from_addr))
    print("toaddr is {}".format(to_addr))
    print("gmail_password is {}".format(gmail_password))
...
def send_mail():
    '''
    Simple mail program to send a backup message in case the SMS service is down.
    It will also send the log file withinformation upuntilthe alarm.
    '''
    msg = MIMEMultipart()
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "Dad_watch ALARM!"
        ...
        while True:
            ...
            for r in results:
                ...
                if side is not old_side:
                    ...
                    if (ALARM == True) :
                        ALARM = False
                    time.sleep(3) # a little delay to avoid jitters
                    logger.debug("resetting ALARM to {}".format(ALARM))
